...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONObject usgs = new JSONObject(SAMPLE_JSON_RESPONSE);

            // build up a list of Earthquake objects with the corresponding data.
            JSONArray usgsQuakes = usgs.getJSONArray("features");
            for (int i=0; i< usgsQuakes.length(); i++) {
                Double mag = usgsQuakes.getJSONObject(i).getJSONObject("properties").getDouble("mag");
                String place = usgsQuakes.getJSONObject(i).getJSONObject("properties").getString("place");
                Long time = usgsQuakes.getJSONObject(i).getJSONObject("properties").getLong("time");

                earthquakes.add(new Earthquake(place, time, mag));
